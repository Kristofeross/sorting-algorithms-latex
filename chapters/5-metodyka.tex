...
gdzie p oznacza liczbę jednostek wykonawczych. W przypadku Bucket Sort oraz Sample Sort 
...
sprofilowanie uruchomionego algorytmu, dzięki czemu narzut pracy profilera nie wpływał na wyniki.

Po każdym wykonaniu algorytmu przeprowadzano również weryfikację poprawności uzyskanego wyniku. 
W tym celu tworzono kopię danych wejściowych, dla której wyznaczano wynik referencyjny 
za~pomocą wbudowanej funkcji \texttt{sorted()} języka Python. 
Wynik uzyskany przez badaną implementację porównywano następnie z~wynikiem referencyjnym.
Weryfikację przeprowadzano zarówno dla wariantów sekwencyjnych, jak i~równoległych.
...
Wykorzystanie procesora oraz pamięci operacyjnej monitorowano z wykorzystaniem biblioteki \texttt{psutil}~\cite{bib:Psutil}. 
...
\begin{itemize}
    ...
    \item wykorzystanie pamięci RAM na podstawie wartości rozmiaru pamięci rezydentnej (ang. \textit{Resident Set Size}, RSS)
\end{itemize}
...
a~następnie wraz z~wynikami pomiaru czasu oraz wartościami \textit{speedup} i~\textit{efficiency} zapisywano w~bazie danych.