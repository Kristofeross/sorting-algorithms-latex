...
Dolne ograniczenie złożoności sekwencyjnej dla tej klasy algorytmów wynosi $\Omega(n \log n)$, natomiast w~dalszej części pracy
symbol $O$ będzie stosowany do określania asymptotycznego ograniczenia górnego złożoności analizowanych algorytmów.
...
Współczesne procesory mogą dodatkowo wykorzystywać mechanizmy wielowątkowości sprzętowej, takie jak jednoczesna wielowątkowość 
(ang. \textit{simultaneous multithreading}, SMT). Przykładem takiego rozwiązania jest technologia Intel Hyper-Threading, 
...
Przyjmując, że $f$ oznacza udział części sekwencyjnej programu, natomiast $p$ liczbę jednostek wykonawczych, 
przyspieszenie uzyskiwane w~wyniku równoległego wykonania programu, zgodnie z~prawem Amdahla, można wyrazić za pomocą zależności:
...
Zwiększenie rozmiaru danych przy stałej liczbie jednostek wykonawczych może zwiększać efektywność systemu, 
ponieważ właściwe obliczenia stanowią wtedy większą część całkowitego czasu wykonania.
