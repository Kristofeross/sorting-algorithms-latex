...
Dla każdego algorytmu wykonano pomiary jego wariantu sekwencyjnego oraz wersję równoległą. 
...
Testy równoległe wykonano dla dostępnych konfiguracji liczby rdzeni udostępnianych przez środowisko sprzętowe. 
...
Liczbę powtórzeń ustalono na 10/100 dla każdego wariantu eksperymentu, 
...
liczby wykorzystywanych rdzeni oraz charakterystyki zbioru wejściowego na czas sortowania, zużycie pamięci oraz efektywność zrównoleglenia.
...
Wariant sekwencyjny jest wykonywany na pojedynczym rdzeniu, z wykorzystaniem identycznych danych wejściowych oraz tych samych 
metod pomiaru czasu sortowania, zużycia pamięci RAM i obciążenia procesora co w odpowiadającym mu wariancie równoległym. 
Jednakowe warunki umożliwiają skupienie analizy na rzeczywistych różnicach pomiędzy wariantami i uzyskanymi przez nich wynikami.

\section{Parametry pomiarów}
\label{sec:ParametryPomiarow}

Podczas każdego eksperymentu zbierane są wyniki pozwalające na późniejszą analizę wydajności oraz efektywności algorytmów 
...
Wartości \textit{speedup} oraz \textit{efficiency} dla wariantu sekwenyjnego są pomijane
...
Czas wykonania wyznaczany jest jako różnica wartości licznika zarejestrowanych przed rozpoczęciem i po zakończeniu
...
Dla zbiorów danych o rozmiarze nieprzekraczającym 10~000 odstęp wynosił 0,01 sekundy, natomaist dla pozostałcyh zbiorów 0,05 sekundy.
Podczas każdego pomiaru odczytano:
\begin{itemize}
...
\begin{itemize}
    ...
    działania procesu głównego i podprocesów oraz generowanie typu \textit{flamegraph}
\end{itemize}
...
Każdy pojedynczy przebieg algorytmu uruchamiano w osobnym procesie roboczym,co zapewniało izolajcę pomiaru oraz 
zapewniało zabezpieczenie testów w przypadku awarii lub zawieszeniu badanego algorytmu.
...
a następnie wraz z metrykami czasu, \textit{speedup} i \textit{efficiency} zapisano w bazie danych.
